...
if [ $showdnsconfig = 1 ]; then
    ...
    echo "         {"
    echo "                 type master;"
    echo "                 file \"/etc/named/db.$region\";"
    echo "         };"
    echo "#"
    echo "# Create the zone file on the parent DNS server"
    echo "> ;"
    echo "> ; DNS zone for $region.$domain"
    echo "> ; - Eucalyptus configured to use CLC as DNS server"
    echo ">"
    echo "# cat << EOF > /etc/named/db.$region"
    echo "> $TTL 1H"
    echo "> $ORIGIN $region.$domain"
    echo "> @                       SOA     ns1 root ("
