...
echo "Commands:"
echo
echo "euform-delete-stack --region $user_region ELBDemoStack"

if ! euform-describe-stacks --region $user_region ELBDemoStack 2> /dev/null | grep -s -q "^STACK"; then
    echo
    tput rev
    ...
    if [ $choice = y ]; then
        echo
        echo "# euform-delete-stack --region $user_region ELBDemoStack"
        euform-delete-stack --region $user_region ELBDemoStack
...
case $(uname) in
  Darwin)
    echo "Eucalyptus CloudFormation ELB demo reset complete (time: $(date -u -r $((end-start)) +"%T"))";;
  *)
    echo "Eucalyptus CloudFormation ELB demo reset complete (time: $(date -u -d @$((end-start)) +"%T"))";;
esac
