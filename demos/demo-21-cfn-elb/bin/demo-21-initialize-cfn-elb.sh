...
case $(uname) in
  Darwin)
    echo "Eucalyptus CloudFormation ELB demo initialization complete (time: $(date -u -r $((end-start)) +"%T"))";;
  *)
    echo "Eucalyptus CloudFormation ELB demo initialization complete (time: $(date -u -d @$((end-start)) +"%T"))";;
esac
