...
if [ $mode = a -o $mode = b ]; then
    ...
    echo
    echo "ssh -t -i ~/.ssh/${aws_ssh_key}_id_rsa $aws_ssh_user@$aws_public_name \\"
    echo "    \"sudo /usr/local/bin/wp post create --path=/var/www/html/wordpress --post_type=\\\"page\\\" --post_status=\\\"publish\\\" --post_title=\\\"Post on $(date)\\\" --post_content=\\\"Post created with wp on $(hostname)\\\"\""
    ...
    if [ $choice = y ]; then
        ...
        while ((attempt++ <= login_attempts)); do
            ...
            ssh -t -i ~/.ssh/${aws_ssh_key}_id_rsa $aws_ssh_user@$aws_public_name \
                "sudo /usr/local/bin/wp post create --path=/var/www/html/wordpress --post_type=\"page\" --post_status=\"publish\" --post_title=\"Post on $(date)\" --post_content=\"Post created with wp on $(hostname)\""
            RC=$?
            if [ $RC = 0 -o $RC = 1 ]; then
                break
            else
