...
if [ $mode = e -o $mode = b ]; then
    ...
    if [ $verbose = 1 ]; then
        ...
        aws ec2 describe-images --filter "Name=manifest-location,Values=images/$image_name.raw.manifest.xml" \
                                --profile $euca_profile --region $euca_region | cut -f1,3,4  | grep -s -q "$image_name" || euca_demo_initialized=n
        aws ec2 describe-key-pairs --filter "Name=key-name,Values=demo" \
...
if [ $mode = a -o $mode = b ]; then
    ...
    echo
    echo "ssh -t -i ~/.ssh/${aws_ssh_key}_id_rsa $aws_ssh_user@$aws_public_name \\"
    echo "    \"sudo /usr/local/bin/wp post create --path=/var/www/html/wordpress --post_type=\\\"post\\\" --post_status=\\\"publish\\\" --post_title=\\\"Post on $(date)\\\" --post_content=\\\"Post created with wp on $(hostname)\\\"\""
    ...
    if [ $choice = y ]; then
        ...
        while ((attempt++ <= login_attempts)); do
            ...
            ssh -t -i ~/.ssh/${aws_ssh_key}_id_rsa $aws_ssh_user@$aws_public_name \
                "sudo /usr/local/bin/wp post create --path=/var/www/html/wordpress --post_type=\"post\" --post_status=\"publish\" --post_title=\"Post on $(date)\" --post_content=\"Post created with wp on $(hostname)\""
            RC=$?
            if [ $RC = 0 -o $RC = 1 ]; then
                break
            else
