...
usage () {
    echo "Usage: ${BASH_SOURCE##*/} [-I [-s | -f]] [-r region ] [-a account] [-A aws_account] [-u user]"
    echo "  -I              non-interactive"
    echo "  -s              slower: increase pauses by 25%"
    echo "  -f              faster: reduce pauses by 25%"
    echo "  -r region       Region (default: $region)"
    echo "  -a account      Account (default: $account)"
    echo "  -A aws_account  AWS Account with template Bucket (default: $aws_account)"
...
while getopts Isfr:a:A:u:? arg; do
    case $arg in
    I)  interactive=0;;
